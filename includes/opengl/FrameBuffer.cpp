...
FrameBuffer::FrameBuffer(const glm::ivec2& size)
    : m_Size(size), Texture(size, GL_RGBA), NormalTexture(size, GL_RGB), PositionTexture(size, GL_RGB), DepthTexture(size, GL_DEPTH_COMPONENT)
{
    ...
    GLCall(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, PositionTexture.m_RendererID, 0));
